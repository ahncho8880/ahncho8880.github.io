...
Java·Spring 기반의 백엔드 시스템을 설계·개발하고 운영해 왔습니다.<br>
운영 환경에서 발생하는 외부 시스템 응답 지연, 동시성 문제, 스레드·DB 자원 고갈을 분석하고 처리 구조와 트랜잭션 범위를 개선해 왔습니다.

- **요청 처리 구조 고도화** — 블록체인이 수용 가능한 처리량을 고려해 배치 크기와 동시 실행 한도를 조정하고, I/O 대기 중 플랫폼 스레드 점유 비용을 줄이기 위해 Virtual Thread 기반 실행 구조로 전환
- **운영 안정화** — 트래픽·응답 지연·에러·리소스 사용량을 모니터링해 병목과 장애 원인을 분석하고, 실행 구조와 자원 사용 방식을 개선
- **트랜잭션 최적화·정합성 설계** — 외부 I/O 대기를 DB 트랜잭션에서 분리해 자원 점유 시간을 줄이고, 상태 관리와 동시성 제어로 단계 간 데이터 정합성을 유지
...
1. 블록체인 Write 트랜잭션 미들웨어 처리 구조의 고도화 과정
   ...
   - 1-2. 블록체인 배치 응답의 무기한 대기로 인한 공유 비동기 스레드 풀 고갈 개선
...
- 최대 20건의 Write 요청 트랜잭션을 단일 배치 트랜잭션으로 처리했다.
...
당시 블록체인 배치 결과를 기다리는 보상 작업과 일반 비동기 작업은 하나의 플랫폼 스레드 풀을 공유했다. 운영 중 보상 작업이 공유 풀의 스레드를 모두 점유해 일반 비동기 작업도 실행되지 못했다. 장애 당시 스레드 덤프에서 `CompletableFuture.join()`의 `WAITING (parking)` 상태를 다수 확인했다.
...
> 60초 타임아웃은 온체인 실패나 취소를 의미하지 않는다. 대기 큐는 무제한 메모리 큐로, 적체 시 메모리 사용량이 증가하고 종료 시 대기 작업이 유실될 수 있다. 큐 한도 초과 시 거절된 요청을 처리하는 경로가 없어 무제한 큐를 유지했다.
...
#### 핵심 구현 · 플랫폼 스레드 대비 변경점

| 스레드 종류 | 플랫폼 스레드 | 버추얼 스레드 |
|---|---|---|
...
전체 소요 시간 약 90초에는 Runnable 큐에서 실행을 기다린 시간이 포함됐다. 실행 전 큐 대기는 개별 Future의 60초 타임아웃에 포함되지 않으며, 해당 제한은 블록체인 배치 큐에 요청을 적재한 뒤 결과를 기다리는 구간에 적용했다.
